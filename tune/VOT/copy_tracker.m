        ...
        for output_sigma_factor = 0.1:0.1
            for numScale = 3:5
                for interp_factor = 0:0.002:0.02
                    ...
                    modifiedStr = strrep(report, 'DCFNet_n', ['DCFNet_',num2str(i)]);
...
for j = i:-1:1
    filename = ['run_experiments',num2str(j)];
    eval(filename);
end
...
for net_index = 6:6
    for scale_penalty = 0.97:0.005:1
        for output_sigma_factor = 0.1:0.1
            for numScale = 3:3
                for interp_factor = 0:0.002:0.02
                    ii = ii+1;
                    filename = ['tracker_DCFNet',num2str(ii),'.m'];
                    fid = fopen(filename, 'w');
                    modifiedStr = strrep(report, 'DCFNet_n', ['DCFNet_',num2str(ii)]);
                    modifiedStr = strrep(modifiedStr, 'net_index = 6', ['net_index = ',num2str(net_index)]);
                    modifiedStr = strrep(modifiedStr, 'interp_factor = 0.01', ['interp_factor = ',num2str(interp_factor)]);
                    modifiedStr = strrep(modifiedStr, 'numScale = 3', ['numScale = ',num2str(numScale)]);
                    modifiedStr = strrep(modifiedStr, 'output_sigma_factor = 0.1', ['output_sigma_factor = ',num2str(output_sigma_factor)]);
