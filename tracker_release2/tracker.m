...
for frame = 1:numel(img_files),
    ...
    if frame > 1,
        ...
        zf = fft2(get_features(patch, features, cell_size, cos_window, net));
    ...
    patch = get_subwindow(im, pos, window_sz);
    xf = fft2(get_features(patch, features, cell_size, cos_window, net));
...
function net = init_rand_net()
net = [];
net.layers{1} = struct(...
    'name', 'conv1', ...
    'type', 'conv', ...
    'weights', {{randn(3,3,3,16,'single'), randn(16,1,'single')}}, ...
    'pad', 1, ...
    'stride', 1) ;
net.layers{2} = struct(...
    'name', 'sigmoid1', ...
